    ...
    <div class="main_body flex-column">
        <div class="main_body flex-column overflow-y-auto m">
            ...
            <el-table :data="list?.data" stripe style="width: 100%" v-loading="loading">
                ...
                <el-table-column prop="goods.name" fixed label="名称" width="200" />
                <el-table-column prop="no" label="编号" width="100" />
                <el-table-column prop="type" label="类型" >
                    <template #default="scope">
                        <el-tag effect="dark" :type="scope.row.type == 1?'success':'danger'" >{{scope.row.type == 1?'首发':'寄售'}}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="user.name" label="下单用户" />
                <el-table-column prop="seller_username" label="卖家" />
